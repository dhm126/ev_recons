...
def main():
    ...
    parser.add_argument("--event_file", help="Events in 'events' are of type (x, y, t, p)"
                        ...
                        default="/home/simon/data/TUM-VIE/mocap-shake2/events_right.h5")

    args = parser.parse_args()
    rawfile = Path(args.event_file)
    h5file = h5py.File(rawfile)
    print(h5file.keys())
    events = h5file['events']#dataspace 
    ...
    # Option2: Alternatively, you can use the code from DSECÂ´s event data tool,
    ...
    # DSECÂ´s tools also allow to use an EventSlicer object for reading in specific time interval
    slicer = EventSlicer(h5file)
    # get exemplary event batch between (15ms, 20ms)
    evs = slicer.get_events(15e3, 20e3)
